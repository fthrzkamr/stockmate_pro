...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nama) {
        ...
        // Cek duplikat
        $cek = $conn->prepare("SELECT COUNT(*) FROM bagian WHERE LOWER(nama_bagian) = LOWER(?)");
        $cek->execute([$nama]);
        if ($cek->fetchColumn() > 0) {
            $error = "Bagian <b>$nama</b> sudah ada.";
        } else {
            try {
                $conn->prepare("INSERT INTO bagian (nama_bagian) VALUES (?)")->execute([$nama]);
                $_SESSION['flash_success'] = "Bagian <b>$nama</b> berhasil ditambahkan.";
                $success = true;
            } catch(Exception $e) {
...
<div class="fade-up max-w-lg mx-auto space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-800">Tambah Bagian</h1>
            <p class="text-slate-500 text-sm">Tambah departemen / divisi baru.</p>
        </div>
    </div>
    ...
    <form method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            <div>
                ...
                <input type="text" name="nama_bagian" value="<?= htmlspecialchars($_POST['nama_bagian'] ?? '') ?>"
...
<script>
<?php if ($success): ?>
Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Bagian baru berhasil ditambahkan.', timer: 1500, showConfirmButton: false })
