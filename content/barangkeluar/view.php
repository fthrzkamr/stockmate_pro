...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'cancel_transaction') {
    ...
    try {
        ...
        // Hapus data transaksi barang keluar diganti menjadi update status
        foreach ($items as $trx) {
            ...
            writeAuditLog(
                ...
                'barang_keluar', 
                $trx['id_keluar'], 
                "Mengembalikan stok ke inventory: {$trx['nama_barang']} (Qty: {$trx['qty']})"
            );
        }

        // Now update status instead of delete
        $upd = $conn->prepare("UPDATE barang_keluar SET status = 'Dikembalikan' WHERE COALESCE(kode_transaksi, id_keluar) = ?");
        $upd->execute([$trx_code]);

        $conn->commit();
        echo json_encode(['status' => 'success', 'msg' => "Transaksi dibatalkan dan stok telah dikembalikan ke Inventory."]);
    } catch (Exception $e) {
        $conn->rollBack();
...
if (!canDo('barangkeluar', 'view')) exit;
global $conn;
...
<div class="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white flex items-center justify-between">
    <div class="flex items-center gap-3">
        <div class="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center shadow-sm border border-indigo-100/50">
            <i class="fa-solid fa-truck-fast text-xl animate-pulse"></i>
        </div>
        <div>
            <div class="flex items-center gap-2">
                <h3 class="text-lg font-bold text-slate-800">Detail Pengiriman</h3>
                <?php
                $status = $first['status'] ?? 'Pending';
                $statusColor = 'amber';
                if ($status === 'Diterima') $statusColor = 'emerald';
                if ($status === 'Ditolak') $statusColor = 'rose';
                if ($status === 'Dikembalikan') $statusColor = 'slate';
                ?>
...
<div class="p-6 space-y-6">
    <?php if ($first['status'] === 'Ditolak'): ?>
    <div class="bg-rose-50 border border-rose-200 p-4 rounded-2xl flex items-start gap-3 shadow-sm">
        <div class="w-8 h-8 bg-rose-500/10 text-rose-600 rounded-xl flex items-center justify-center shrink-0">
            <i class="fa-solid fa-triangle-exclamation text-sm"></i>
        </div>
        <div>
            <p class="text-[11px] font-bold text-rose-700 uppercase tracking-wider mb-0.5">Pengiriman Ditolak</p>
            <p class="text-xs text-rose-600 leading-relaxed">Pengiriman ini telah <b>ditolak</b> oleh pihak outlet. Silakan periksa kembali atau tekan tombol <b>Kembalikan ke Inventory</b> di bawah untuk mengembalikan stok ke Gudang Pusat.</p>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div>
        <div class="flex items-center justify-between mb-3">
            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Daftar Barang Dikirim</p>
            ...
            <table class="w-full text-left text-sm border-collapse">
                ...
                <tbody class="divide-y divide-slate-150">
                    <?php foreach ($items as $i): 
                        ...
                        elseif ($i['status'] === 'Ditolak') $color = 'rose';
                        elseif ($i['status'] === 'Dikembalikan') $color = 'slate';
                    ?>
                ...
                <tfoot>
                    <tr class="bg-slate-50/80 border-t-2 border-slate-200 font-bold">
                        ...
                        <td class="px-5 py-4 text-right font-mono text-base text-slate-800" colspan="2"><?= number_format($total_qty) ?> Pcs</td>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    <div class="bg-amber-50/50 border border-amber-200/60 p-4 rounded-2xl flex gap-3 shadow-sm shadow-amber-50/20">
        ...
        <div>
            <p class="text-[10px] font-bold text-amber-650 uppercase tracking-wider mb-1">Catatan</p>
