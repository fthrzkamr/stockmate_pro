...
$whereClauses = ["1=1", "so.periode_bulan IS NULL"];
$params = [];
...
try {
    ...
    $query = "
        ...
        LEFT JOIN users u ON so.id_user = u.id_user
        WHERE $whereSql
        GROUP BY so.no_so, so.tanggal, so.status_approval, so.catatan_reject, o.nama_outlet, u.nama
        ORDER BY so.tanggal DESC, max_id_so DESC
...
<div class="fade-up max-w-7xl mx-auto space-y-5">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Stock Opname Harian</h1>
            <p class="text-slate-500 text-sm mt-1">Kelola dan input perhitungan stok fisik harian di outlet.</p>
        </div>
        <?php if (canDo('stockopname', 'create')): ?>
        <div class="flex gap-2">
            <a href="<?= $sistem ?>/stockopname/i" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-sm">
                <i class="fa-solid fa-plus"></i> Input Stok Akhir
            </a>
        </div>
        <?php endif; ?>
    </div>
...
?>
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-3">
        <form id="filterForm" method="GET" class="flex flex-col sm:flex-row w-full gap-3">
            <input type="hidden" name="menu" value="stockopname">
            ...
            <a href="<?= $sistem ?>/stockopname" class="bg-slate-100 text-slate-600 hover:bg-slate-200 px-5 py-2 rounded-xl text-sm font-semibold transition-colors whitespace-nowrap">
                Reset
            </a>
            <?php endif; ?>
        </form>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="flex items-center justify-end px-6 py-3.5 border-b border-slate-100 bg-slate-50/20">
            <?php echo generateShowEntries($limit, 'stockopname', $search); ?>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr>
                        <td colspan="<?= $my_outlet_id ? '7' : '8' ?>" class="px-6 py-12 text-center">
                            ...
                            <h3 class="text-slate-800 font-bold mb-1">Belum Ada Data Stock Opname</h3>
                            <p class="text-slate-500 text-sm">Catatan perhitungan stock opname harian akan tampil di sini.</p>
                        </td>
                    </tr>
                    <?php else: ?>
                        <?php 
                        ...
                        <tr class="hover:bg-slate-50/70 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <?php if ($status === 'Rejected' && canDo('stockopname', 'edit')): ?>
                                    <a href="<?= $sistem ?>/stockopname/e/<?= $row['no_so'] ?>"
                                    ...
                                    <?php if ($status === 'Pending' && canDo('stockopname', 'delete')): ?>
        ...
        <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-1">
                <button type="button" onclick="goToPage(<?= max(1, $page - 1) ?>, <?= $limit ?>, 'stockopname', '<?= urlencode($search) ?>')"
                ...
                <div class="flex items-center px-2 gap-1 hidden sm:flex">
                    ...
                    for ($i = $start_page; $i <= $end_page; $i++) {
                        if ($i == $page) {
                            ...
                            echo '<button type="button" onclick="goToPage('.$i.', '.$limit.', \'stockopname\', \''.urlencode($search).'\')" class="w-8 h-8 rounded-lg text-sm font-semibold text-slate-600 hover:bg-slate-100 transition-colors">'.$i.'</button>';
                        }
                    }
                    ?>
                </div>

                <button type="button" onclick="goToPage(<?= min($totalPages, $page + 1) ?>, <?= $limit ?>, 'stockopname', '<?= urlencode($search) ?>')"
...
function cancelSO(noSO) {
    Swal.fire({
        title: 'Hapus Pengajuan SO?',
        html: `Apakah Anda yakin ingin menghapus lembar stock opname <b>${noSO}</b>?<br><span class="text-xs text-rose-500 font-semibold">*Data pengajuan ini akan dihapus permanen.</span>`,
        icon: 'warning',
        ...
        fetch('<?= $sistem ?>/stockopname/v/' + noSO, { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(r => r.json())
            .then(res => {
